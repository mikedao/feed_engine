...
RSpec.describe Api::V1::TweetsController, type: :controller do
  describe "index" do
    it "returns all tweets without params" do
      ...
      expect(parsed_tweets.first["body"]).to eq(tweet3.body)
      expect(parsed_tweets.first["username"]).to eq(tweet3.username)
      expect(parsed_tweets.last["body"]).to eq(tweet1.body)
      expect(parsed_tweets.last["username"]).to eq(tweet1.username)
    end
    ...
    it "returns tweets with article id as params" do
      ...
      article.tweets.create(body: "Some tweet",
                            username: "bmrsny",
                            tweet_id: 111)
      test_article = article.tweets.create(body: "Some other tweet",
                                           username: "j3",
                                           tweet_id: 222)
      ...
      expect(body.first["body"]).to eq(test_article.body)
      expect(body.first["username"]).to eq(test_article.username)
